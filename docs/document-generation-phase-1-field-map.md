...
| Application Form | Recommended amount, term, interest context | `loan_requests` | `recommended_amount`, `recommended_term`, `recommended_interest_rate`, `recommended_payment_frequency` | Processor-owned | Required before recommendation | Currency/term labels from blade partial | Missing recommendation data blocks recommendation, not simple preview |
...
| Affidavit of Undertaking | Witness metadata | Stored in `loan_request_data_entries` but not rendered by the checked-in template map | witness fields | Staff-owned | Not currently required | No verified official slot in the checked-in PDF map | Documented template-slot blocker; do not guess placement |
