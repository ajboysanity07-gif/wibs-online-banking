...
test('admin can view loan payments print preview', function () {
    ...
    $response->assertSee('This is a system-generated Loan Payment Report prepared from the official loan payment records');
    $response->assertSee('Document Control');
    $response->assertSee('Generated on:');
    $response->assertSee('Generated by:');
    $response->assertSee('Report period:');
    $response->assertSee('Loan number:');
    $response->assertSee('Member account no:');
    $response->assertSee('Validation Notice');
...
test('opening balance derives from earliest included transaction for all range', function () {
    $member = User::factory()->create(['acctno' => '000930']);
    ...
    DB::table('wlnled')->insert([
        'acctno' => $member->acctno,
        'lnnumber' => 'LN-930',
        'date_in' => Carbon::parse('2024-01-01 00:00:00')->toDateTimeString(),
        'principal' => 100,
        'payments' => 0,
        'balance' => 100,
    ]);
    ...
    $payload = app(MemberLoanService::class)->getPaymentsExportData(
        $member,
        'LN-930',
        'all',
        null,
    ...
    expect($payload['openingBalance'])->toBe(0.0);
    expect($payload['closingBalance'])->toBe(80.0);
});

test('opening balance derives from first in-range transaction when no prior row exists', function () {
    $member = User::factory()->create(['acctno' => '000931']);
    ...
    DB::table('wlnled')->insert([
        'acctno' => $member->acctno,
        'lnnumber' => 'LN-931',
        'date_in' => Carbon::parse('2024-02-15 00:00:00')->toDateTimeString(),
        'principal' => 0,
        'payments' => 50,
        'balance' => 750,
    ]);
    ...
    expect($payload['openingBalance'])->toBe(600.0);
    expect($payload['closingBalance'])->toBe(750.0);
});

test('opening balance prefers prior balance over derived fallback', function () {
    $member = User::factory()->create(['acctno' => '000932']);
    ...
    DB::table('wlnled')->insert([
        'acctno' => $member->acctno,
        'lnnumber' => 'LN-932',
        'date_in' => Carbon::parse('2024-01-15 00:00:00')->toDateTimeString(),
        'principal' => 100,
        'payments' => 0,
        'balance' => 900,
    ]);
    ...
    expect($payload['openingBalance'])->toBe(900.0);
    expect($payload['closingBalance'])->toBe(800.0);
});

test('pdf export uses derived opening balance when no prior row exists', function () {
    $member = User::factory()->create(['acctno' => '000933']);
    ...
    Pdf::shouldReceive('loadView')
        ->once()
        ->with('reports.loan-payments', \Mockery::on(function (array $data) {
            return isset($data['openingBalance'])
                && $data['openingBalance'] === 400.0;
        }))
        ->andReturnSelf();
...
test('loan payment report template uses simplified columns', function () {
    ...
    expect($html)
        ...
        ->toContain('This is a system-generated Loan Payment Report prepared from the official loan payment records')
        ->toContain('Document Control')
        ->toContain('Generated on:')
        ->toContain('Generated by:')
        ->toContain('Report period:')
        ->toContain('Loan number:')
        ->toContain('Member account no:')
        ->toContain('Validation Notice')
        ->toContain('This document is electronically generated and is valid without handwritten signature')
