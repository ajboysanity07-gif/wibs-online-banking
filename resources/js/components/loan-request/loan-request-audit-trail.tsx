...
export function LoanRequestAuditTrail({
    ...
    const renderEntry = (entry: LoanRequestAuditEntry, isLatest: boolean) => (
        <div key={`${entry.action}-${entry.id}`} className="flex gap-3">
            <div className="flex w-6 shrink-0 items-start justify-center">
                <span
                    className={cn(
                        'relative z-10 mt-1.5 size-3 rounded-full border',
                        isLatest
            ...
            <div className="min-w-0 flex-1 space-y-2 pb-0.5">
                ...
                {showStaffMetadata && entry.metadata.length > 0 ? (
            ...
            <Collapsible open={open} onOpenChange={setOpen}>
                <CollapsibleTrigger asChild disabled={!hasOlderEntries}>
                    <CardHeader
                        className={cn(
                            'select-none',
                            hasOlderEntries && 'cursor-pointer',
                        )}
                    >
                ...
                <CardContent>
                    {entries.length === 0 ? (
                        ...
                        <div className="relative">
                            ...
                            <div>{renderEntry(latestEntry, true)}</div>
                            {hasOlderEntries ? (
