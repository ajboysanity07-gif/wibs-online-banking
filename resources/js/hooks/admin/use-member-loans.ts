...
import type { MemberLoansResponse, PaginationMeta } from '@/types/admin';

type MemberLoansState = {
    data: MemberLoansResponse;
    loading: boolean;
    error: string | null;
};

type MemberLoansOptions = {
    enabled?: boolean;
    initial?: MemberLoansResponse;
};

const buildEmptyResponse = (perPage: number): MemberLoansResponse => ({
    items: [],
    meta: {
        page: 1,
        perPage,
